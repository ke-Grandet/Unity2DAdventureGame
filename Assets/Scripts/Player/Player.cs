...
public class Player : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Vector2 rayOrigin = new(_rigidbody.position.x - _capsuleCollider.bounds.size.x / 2,
            _rigidbody.position.y - _spriteRenderer.bounds.size.y / 2);
        RaycastHit2D raycastHit = Physics2D.Raycast(
            rayOrigin, Vector2.right, _capsuleCollider.bounds.size.x, groundLayer);
        // 如果踩到地板且剩余跳跃段数不为满值
        if (raycastHit.collider != null && jumpRemainingTimes < jumpTimes)
        {
            // 充满跳跃段数
            jumpRemainingTimes = jumpTimes;
        }
    ...
    private void FixedUpdate()
    {
        ...
        // 跳跃
        if (jumpRequest && jumpRemainingTimes > 0)
        {
            // 清零垂直速度
